...
Controller.prototype.render = function(template, options, fn) {
  ...
  if (!this.__res.get('Content-Type') && !fn) {
    ...
    this.__res.set('Content-Type', type);
  }
  
  // delete option keys consumed by Maglev
 ...
 *     })
 *
 * By default Maglev passes an `Error` with a `.status` of 406 to
...
/**
 * Invoked when controller is initialized by Maglev.
 *
 * This function is called immediately after Maglev instantiates the
...
Controller.prototype._prepare = function(req, res, next) {
  this.app = this.__app;
  this.req = this.request = this.__req = req;
  this.res = this.response = this.__res = res;
...
Controller.prototype._invoke = function(action) {
  ...
  // Assign Maglev properties to request.  These properties are used by
