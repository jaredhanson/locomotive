...
exports = module.exports = function server(dir, address, port, env, options) {
  ...
  if (cluster.isMaster) {
console.log('Locomotive %s application starting in %s on http://%s:%d', locomotive.version, env, address, port);
  // Fork workers.
  ...
  cluster.on('exit', function(worker, code, signal) {
    ...
    console.log('worker #' + worker.id +" PID:"+ worker.process.pid + ' died ('+exitCode+') dont worry restarting...');
    cluster.fork();
  });
