...
exports = module.exports = function server(dir, address, port, env, options) {
  ...
  var app = locomotive;
  app.phase(locomotive.boot.controllers(path.resolve(dir, './app/controllers'), app));
  app.phase(locomotive.boot.views(app));
  app.phase(require('bootable-environment')({ dirname: path.resolve(dir, './config/environments'), env: env }, app));
  app.phase(bootable.initializers(path.resolve(dir, './config/initializers'), app));
  app.phase(require('../boot/datastores')(app));
  app.phase(locomotive.boot.routes(path.resolve(dir, './config/routes'), app));
  app.phase(locomotive.boot.httpServer({ address: address, port: port }, app));
