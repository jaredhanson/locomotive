...
Router.prototype.match = function(pattern, shorthand, options) {
  ...
  if (options.as) {
    ...
    var helpers = {};
    helpers[helper + 'Path'] = pathHelper(path);
    var dynamicHelpers = {};
    dynamicHelpers[helper + 'URL'] = urlHelper(path);
...
Router.prototype._route = function(method, pattern, controller, action) {
  ...
  this._http[method](pattern, this._handle(controller, action).bind(this.app));
}

/**
 * Create route to `controller` and `action`.
 * 
 * @param {String} controller
 * @param {String} action
 * @return {Route}
