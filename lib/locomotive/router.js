...
Router.prototype.resources = function(name, fn) {
  ...
  this._route('get' , path + '/:id/edit'     , controller, 'edit'    , { helper: 'edit' + inflect.capitalize(helper) });
  ...
  this.namespace(name + '/' + placeholder, { module: null, helper: helper }, function() {
    fn && fn.call(this);
  });
}
...
Router.prototype._route = function(method, pattern, controller, action, options) {
  ...
  if (options.helper) {
    // TODO: Register helpers with helper(name, fn), rather than object.
    var helper = options.helper;
    var helpers = {};
    helpers[helper + 'Path'] = route.helper();
    var dynamicHelpers = {};
    dynamicHelpers[helper + 'URL'] = route.helper(true);
    
    this._app.helpers(helpers);
    this._app.dynamicHelpers(dynamicHelpers);
  }
}
