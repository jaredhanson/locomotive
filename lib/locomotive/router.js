...
Router.prototype.resource = function(name, fn) {
  var ns = this._ns[this._ns.length - 1]
    , path
    , controller = inflect._controllerize(name);
  
  path = ns.qpath(name);
  controller = ns.qmodule(controller);
  
  // TODO: Ensure that optional format parameters are available and handled
  //       correctly.
  
  var helper = inflect.camelize(name);
  
  this._route('get' , path + '/new'     , controller, 'new'     , { helper: 'new' + inflect.capitalize(helper) });
  this._route('post', path              , controller, 'create' );
  this._route('get' , path + '.:format?', controller, 'show'    , { helper: helper });
  this._route('get' , path + '/edit'    , controller, 'edit'    , { helper: 'edit' + inflect.capitalize(helper) });
  this._route('put' , path              , controller, 'update' );
  this._route('del' , path              , controller, 'destroy');
  
  this.namespace(name, { module: null }, function() {
    fn && fn.call(this);
  });
}
...
Router.prototype.resources = function(name, fn) {
  ...
  var indexHelper = inflect.camelize(name);
  var itemHelper = inflect.en.singularize(indexHelper);
  
  // TODO: Ensure that optional format parameters are available and handled
  //       correctly.
  
  this._route('get' , path                   , controller, 'index'   , { helper: indexHelper });
  this._route('get' , path + '/new'          , controller, 'new'     , { helper: 'new' + inflect.capitalize(itemHelper) });
  this._route('post', path                   , controller, 'create' );
  this._route('get' , path + '/:id.:format?' , controller, 'show'    , { helper: itemHelper });
  this._route('get' , path + '/:id/edit'     , controller, 'edit'    , { helper: 'edit' + inflect.capitalize(itemHelper) });
  this._route('put' , path + '/:id'          , controller, 'update' );
  this._route('del' , path + '/:id'          , controller, 'destroy');

  // TODO: Should camelize this
  var placeholder = ':' + inflect.en.singularize(name) + 'ID';
  this.namespace(name + '/' + placeholder, { module: null }, function() {
    fn && fn.call(this);
  });
}
