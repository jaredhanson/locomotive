...
Router.prototype.match = function(pattern, shorthand, options) {
  ...
  // TODO: Can get rid of this check, its handled by namespace qualifiers
  var path = (pattern[0] === '/') ? pattern : '/' + pattern;
  this._route(method, path, controller, action, { helper: options.as });
}
...
Router.prototype.resource = function(name, fn) {
  ...
  path = ns.qpath(name);
  controller = ns.qcontroller(controller);
