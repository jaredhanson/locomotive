...
Router.prototype.match = function(pattern, shorthand, options) {
  ...
  // TODO: Lowercase the via, so it can be specified as 'POST' (or whatever)
...
Router.prototype.resource = function(name, fn) {
  var ns = this._ns[this._ns.length - 1]
    , path = ns.qpath(name)
    , controller = ns.qcontroller(utils.controllerize(name))
    , helper = ns.qhelper(utils.helperize(name));
...
Router.prototype.resources = function(name, fn) {
  var ns = this._ns[this._ns.length - 1]
    , path = ns.qpath(name)
    , controller = ns.qcontroller(utils.controllerize(name))
    , helper = ns.qhelper(inflect.en.singularize(utils.helperize(name)))
    , collectionHelper = ns.qhelper(utils.helperize(name))
    , placeholder;
  ...
  helper = inflect.en.singularize(utils.helperize(name));
