...
Locomotive.prototype.boot = function(dir, env, options, callback) {
  ...
  options = options || {};
  options.controllersDir = options.controllersDir || path.resolve('./app/controllers');
  options.environmentsDir = options.environmentsDir || path.resolve('./config/environments');
  options.initializersDir = options.initializersDir || path.resolve('./config/initializers');
  options.routesFile = options.routesFile || path.resolve('./config/routes.js');
  ...
  // This is what actually controls the booting order
  async.series([
      configure,
      initialize,
      defaults,
