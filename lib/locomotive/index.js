...
Locomotive.prototype._boot = function(dir) {
  ...
  if (path.existsSync(entry)) {
    var re = RegExp('^' + entry.replace(/\//g, '\\/')+'\\/');
    diveSync(entry, function (err, fullFilename) {
      if (/\.js$/.test(fullFilename)) {
        var filename = fullFilename.replace(re, '');
        var name = filename.replace(/\.js$/, '');
        self.controller(name, require(fullFilename));
      }
    });
  }
}
