 ...
 * @api public
 */
Locomotive.prototype.boot = function(dir, env, callback) {
  var self = this;
  var entry;
  ...
  // Forward function calls from Locomotive to Express.
  utils.forward(this, server, [ 'set', 'enabled', 'disabled', 'enable', 'disable',
                                'use', 'register' ]);
  this.router = server.router;
  
  
  // TODO: Should set('env', env) here on the underlying Express server
  ...
  if (path.existsSync(entry)) {
    ...
    fs.readdirSync(entry).sort().forEach(function(filename) {
      if (/\.js$/.test(filename)) {
        debug('requiring initializer: %s', filename);
  ...
  entry = configDir + '/environments/' + env + '.js';
  if (path.existsSync(entry)) {
  ...
  if (path.existsSync(entry)) {
    diveSync(entry, function (err, path) {
      if (/\.js$/.test(path)) {
        var name = path.slice(entry.length + 1).replace(/\.js$/, '');
