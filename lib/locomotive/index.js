...
Locomotive.prototype.boot = function(dir, env, options, callback) {
  ...
  options.coffeeScript = options.coffeeScript || false;
  if (options.coffeeScript) {
    debug('Enabling .coffee (CoffeeScript) file types.');
  }
  ...
  function environments(done) {
    ...
    if (options.coffeeScript && !existsSync(file)) {
      file = path.join(dir, 'all.coffee');
    }
    if (existsSync(file)) {
  ...
  function initializers(done) {
    ...
    async.forEachSeries(files, function(file, next) {
      var filetypeRegex = /\.js$/;
      if (options.coffeeScript) {
        filetypeRegex = /\.(js|coffee)$/;
      }      
      if (filetypeRegex.test(file)) {
  ...
  function controllers(done) {
    ...
    diveSync(dir, function(err, path) {
      ...
      if (filetypeRegex.test(path)) {
        var name = path.slice(dir.length + 1).replace(filetypeRegex, '');
  ...
  function routes(done) {
    var file = options.routesFile + ".js";
    if (options.coffeeScript && !existsSync(file)) {
      file = options.routesFile + ".coffee";
    }
    if (existsSync(file)) {
