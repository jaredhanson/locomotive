...
// TODO: Implement shorter function signature:
//       this.render('feed', 'xml'); => this.render('feed', { format: 'xml' });
...
Controller.prototype.render = function(template, options, fn) {
  ...
  // TODO: Automatically set content-type in all cases, if not explicitly set (???)
  // TODO: Set charset
  if (options.format) {
    this.__res.setHeader('Content-Type', utils.normalizeType(options.format).value);
  }
...
// TODO: Implement shorter function signature:
//       this.respond(['xml', 'json']);
//       this.respond('xml', 'json');
//       this.respond({ 'json': true, default: 'xml' })
...
Controller.prototype.respondWith = function(obj) {
  ...
  // TODO: Append, if Vary already set
  ...
  (function iter(err, data) {
    ...
    try {
      ...
      if (arity > 2) {
        ...
        filter.fn.call(self, self.req, self.res, iter, data);
      } else {
        filter.fn.call(self, iter, data);
      }
    } catch (ex) {
      iter(ex);
    }
