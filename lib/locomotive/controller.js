...
Controller.prototype.render = function(template, options, fn) {
  ...
  var eng = options.engine || (app && app.set('view engine')) || 'ejs';
	var	hfivfn = options.hideFormatInViewFilename || (app && app.set('hideFormatInViewFilename')) || false;
  ...
  Object.keys(this).filter(localProperties).forEach(function(key) {
	...
	var view = (!hfivfn) ? tmpl + '.' + fmt + '.' + eng : tmpl + '.' + eng
...
Controller.prototype.respondWith = function(obj) {
  ...
  if (key) {
    // TODO: Set Content-Type header.  Best to do this in render() ???
    //res.setHeader('Content-Type', utils.normalizeType(key));
    op = obj[key];
  }
  
  if (typeof op == 'object') {
    // TODO: Implement defaults and call render.
...
// TODO: Implement send() method to send raw response.
