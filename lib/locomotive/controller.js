...
Controller.prototype.render = function(options) {
  ...
  var action = options.action || inflect.underscore(this.__action);
  var format = options.format || 'html';
  var engine = options.engine || (app && app.set('view engine')) || 'ejs';
  ...
  var view = this.__viewDir + '/' + action + '.' + format + '.' + engine;
