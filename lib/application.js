 ...
 * `Maglev` constructor.
 *
 * A default `Maglev` singleton is exported via the module.  Applications
 ...
 *
 * Maglev provides support for content negotiation, allowing a single route
 ...
 *
 * By default, Maglev looks for template files using a convention of
 ...
 *
 * To facilitate mapping models to controllers, Maglev introspects models
 ...
 * should be registered with Maglev to provide the necessary introspection
 * logic.
 *
 * @param {Module} store
 * @api public
 ...
 * a specific controller and action in this Maglev appliction.  This is
 ...
 *
 *     this.use(maglev.invoke());
 *
 * If you are mounting multiple Maglev applications in a single server, a
 ...
 *
 * Controller's are auto-required by Maglev.  The value exported by the
 ...
 *
 * Maglev uses an MVC-style router, where routes map from a URL pattern to
...
/**
 * Boot `Maglev` application.
 *
 * Maglev builds on Express, providing a set of conventions for how to
 * organize code and resources on the file system as well as an MVC architecture
 * for structuring code.
 *
 * When booting a Maglev application, the file system conventions are used
