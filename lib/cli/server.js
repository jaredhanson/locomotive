...
exports = module.exports = function server(dir, address, port, env, options) {
  ...
  // If debug mode is enabled, Maglev will respawn using a node process with
  ...
  console.log('Maglev %s application starting in %s on http://%s:%d', maglev.version
                                                                        , env, address, port);
  
  debug('booting app at %s in %s environment', dir, env);
  
  var app = maglev;
  app.phase(maglev.boot.controllers(path.resolve(dir, './app/controllers')));
  ...
  app.phase(maglev.boot.routes(path.resolve(dir, './config/routes')));
  app.phase(maglev.boot.httpServer(port, address));
